"""
Clinical documentation agent: generate clinical documentation from a provider-patient transcript.
"""
import json
...
def log_clinical_doc(details: Dict[str, Any], workflow_run_id: str, db: Session):
    try:
        ...
        db.add(new_doc)
        db.commit()
        print("Logged clinical document for patient_id:", details.get("patient_id"))
    except Exception as e:
        print("Error logging clinical document:", str(e))


def generate_clinical_doc(transcript: str):
    """Use an LLM to generate a SOAP note + coding suggestions."""
    prompt = f"""
